...
class RegulEnv(gym.Env):
    ...
    def __init__(
        ...
        self.observation_space = spaces.Dict({
            "delayed_train": spaces.Box(
                0,
                self._schedule.num_trains-1,
                dtype=int
            ),
    ...
    def reset(self, seed=None, train=None, track_section=None, delay=None):
        ...
        delay = (
            self.np_random.random() * self._max_delay
            if delay is None
            else delay
        )
        track = self.np_random.choice(
            self.schedule.trajectory(self._delayed_train)
            if track_section is None
            else track_section
        )

        self._schedule = \
            self._schedule.add_delay(self._delayed_train, track, delay)
        self._schedule, self._delayed_train = \
            self._schedule.propagate_delay(self._delayed_train)
    ...
    def step(self, action):
        ...
        track_section, other_train = \
            self._schedule.first_conflict(self._delayed_train)
        ...
        self._schedule, self._delayed_train = \
            self._schedule.propagate_delay(last_out)

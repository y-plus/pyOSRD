...
def path_to(self: Self, node_to_reach: str) -> nx.DiGraph:
...
def quick_path(self: Self, node_to_include: str) -> nx.DiGraph:
...
def interlocking_path(self: Self, node_to_include: str) -> nx.DiGraph:
...
class ACOAgent(GrootAgent):
    ...
    def _calculate_interlocking(self: Self, debug: bool) -> Groot:
        ...
        path, groot = interlocking_path(self, '_')
        return groot
    ...
    def calculate_dispatch(self: Self, debug: bool = False) -> Groot:
        ...
        for n in explored_nodes:
            depth = len(n) -1
            missing_children = self.NUM_ACTIONS - len(list(self.tree.successors(n))) if self.tree.nodes[n]['pheromone']==1 else 0
            num_unexplored_nodes += missing_children * (int((1-self.NUM_ACTIONS**(max_depth-depth+1))/(1-self.NUM_ACTIONS)) -1)
        ...
        if self.debug:
            # nx.draw_networkx(self.tree, pos=nx.planar_layout(self.tree))
            self._draw_tree()
        return self.best_groot

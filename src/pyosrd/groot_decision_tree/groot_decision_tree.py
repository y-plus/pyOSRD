...
class GrootDecisionTree(DecisionTree):
    ...
    def create_node_in_branch(
        ...
        if node[-1] not in self.actions:
            raise ValueError(f"Unknown action {node[-1]}")
        ...
        if not branch.current_groot.has_conflicts():
            branch.nodes[node] = self._scorer(branch.current_groot, self.ref_groot)
            return False

        if self._scorer(branch.current_groot, self.ref_groot) > self.best_solution:    
    ...
    def grow(self: Self, nodes: list[NodeIndex]) -> Groot:
        ...
        if self.debug:
                print(' Merging branches and updating best solution')
        for branch in new_branches:
            if self.debug:
                print(' ->',list(branch.nodes))
            self.combine(branch)
...
def grow_branch_through(
    ...
    missing_actions = branch.missing_children_edges(node)

    while not stop:
        if missing_actions:
            new_node = f"{node}{missing_actions[0]}"
            ok = branch.create_node_in_branch(new_node, branch)
            stop = not branch.current_groot.has_conflicts()
            if ok:
                node = new_node
                missing_actions = branch.missing_children_edges(node)

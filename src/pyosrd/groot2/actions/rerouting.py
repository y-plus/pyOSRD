...
def reroute_train_to_avoid_zone(
    ...
    original_times = {train: copy.deepcopy(groot.times[train])}

    tvd_conflict = groot.get_tvd(train, zone)
    next_station = groot.next_station(train, zone)
    next_next_station = groot.next_station(train, next_station) if next_station else None
    prev_station = groot.previous_station(train, zone)
    prev_prev_station = groot.previous_station(train, prev_station) if prev_station else None

    if prev_station is None and zone != groot.path_zones(train)[0]:
        prev_station = groot.path_zones(train)[0]
    if next_station is None and zone != groot.path_zones(train)[-1]:
        next_station = groot.path_zones(train)[-1]

    tvd_prev_station = groot.get_tvd(train, prev_station)
    tvd_next_station = groot.get_tvd(train, next_station)

    tvd_prev_prev_station = (
        groot.get_tvd(train, prev_prev_station)
        if prev_prev_station
        else None
    )
    tvd_next_next_station = (
        groot.get_tvd(train, next_next_station)
        if next_next_station
        else None
    )

    graph = groot.tvds_graph
    ...
    while nx.has_path(subg, source=source, target=target):
        ...
        rerouted_groot = copy.deepcopy(groot)
        ...
        original_zones = [groot.zones[tvd] for tvd in original_path[1:-1]]
        original_station = next((
            z for z in original_zones if '/' in z
        ), None)
        new_zones = [groot.zones[tvd] for tvd in new_path[1:-1]]
        new_station = next((
        ...
        if original_station and new_station:
            ...
            tvd_original_station = groot.get_tvd(train, original_station)
            tvd_new_station = next(tvd for tvd in new_path if groot.zones[tvd]==new_station)
            ...
            t_in_0 = groot.times_zones[train][groot.zones[tvds_before_original_station[0]]][0]
            t_out_0 = groot.times_zones[train][groot.zones[original_path[0]]][1]

            ts_in_0 = groot.times[train][tvd_original_station][0]
            ts_out_0 = groot.times[train][tvds_before_original_station[-1]][1]

            ts_in_1 = groot.times[train][tvds_after_original_station[0]][0]
            ts_out_1 = groot.times[train][tvd_original_station][1]

            t_in_1 = groot.times_zones[train][groot.zones[original_path[-1]]][0]
            t_out_1 = groot.times_zones[train][groot.zones[original_path[-2]]][1]
            ...
            new_length_before_station = sum(
                distance_between_points(
                    ...
                    tvd.split('->')[0],
                    tvd.split('->')[1],
                    groot._track_section_lengths,
                    groot._track_section_network
                )
            ...
            rerouted_groot.times[train][tvd_new_station] = groot.times[train][tvd_original_station]
            ...
            t_in_0 = groot.times_zones[train][groot.zones[original_path[1]]][0]
            t_out_0 = groot.times_zones[train][groot.zones[original_path[0]]][1]
        ...
        if conflict_zone in zones_that_must_be_free and train in (tr1, tr2):
            ...
            groot.times = rerouted_groot.times

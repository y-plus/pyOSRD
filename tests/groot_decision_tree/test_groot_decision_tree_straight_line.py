...
class TestGrootDecisionTree:
    ...
    @pytest.mark.parametrize("action", ['R', 'O'])
    ...
    def test_grow_branches(self, tree: GrootDecisionTree):
        ...
        assert list(branches[1].nodes.keys()) == ['', 'R']

    def test_grow(self, tree: GrootDecisionTree):
        tree.grow(['S', 'R'])
        assert set(tree.nodes.keys()) == {'', 'R', 'S', 'SR', 'SS'}
        assert tree.solution_nodes == {'R', 'SR', "SS"}

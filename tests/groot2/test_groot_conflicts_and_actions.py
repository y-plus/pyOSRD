...
class TestsConflicts:
    ...
    def test_groot2_has_conflicts(self, groot2_2trains):
        assert not groot2_2trains.has_conflicts()
        disrupted = groot2_2trains.add_delay('train1', 'A', 2)
        assert disrupted.has_conflicts()

    def test_groot2_earliest_conflict(self, groot2_2trains):
        disrupted = groot2_2trains.add_delay('train1', 'A', 2)
        assert disrupted.earliest_conflict() == ('train1', 'train2', 'A', 2)

    def test_groot2_earliest_conflict2(self, groot2_2trains):
        disrupted = groot2_2trains.add_delay('train1', 'B', 2)
